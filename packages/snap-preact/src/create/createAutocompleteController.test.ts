...
import { createAutocompleteController } from './index';

import type { SnapAutocompleteControllerConfig } from '../types';
import type { UrlTranslatorConfig } from '@searchspring/snap-url-manager';

const createConfig: SnapAutocompleteControllerConfig = {
	client: {
		globals: {
			siteId: '8uyt2m',
		},
...
describe('createAutocompleteController', () => {
	...
	it('creates an autocomplete controller', () => {
		const controller = createAutocompleteController(createConfig);

		expect(controller).toBeDefined();
		expect(controller.id).toBe(createConfig.controller.id);
		expect(controller.context).toBe(createConfig.context);
		expect(controller.config.selector).toBe(createConfig.controller.selector);
	...
	it('creates an autocomplete controller with custom UrlTranslator config', () => {
		...
		const controller = createAutocompleteController(customUrlConfig as SnapAutocompleteControllerConfig);
	...
	describe('custom services', () => {
		it('creates an autocomplete controller with custom Client service', () => {
			const clientConfig = { siteId: 'custom' };
			const customClient = new Client(clientConfig);

			const controller = createAutocompleteController(createConfig, { client: customClient });
		...
		it('creates an autocomplete controller with custom Store service', () => {
			const storeConfig = {
			...
			const customStore = new AutocompleteStore(storeConfig, { urlManager: customUrlManager });

			const controller = createAutocompleteController(createConfig, { store: customStore });

			expect(controller).toBeDefined();
			expect(controller.store).toBe(customStore);
		});

		it('creates an autocomplete controller with custom UrlManager service', () => {
			...
			const controller = createAutocompleteController(createConfig, { urlManager: customUrlManager });
		...
		it('creates an autocomplete controller with custom EventManager service', () => {
			...
			const controller = createAutocompleteController(createConfig, { eventManager: customEventManager });

			expect(controller).toBeDefined();
			expect(controller.eventManager).toBe(customEventManager);
		});

		it('creates an autocomplete controller with custom Profiler service', () => {
			const customProfiler = new Profiler('customProfiler');

			const controller = createAutocompleteController(createConfig, { profiler: customProfiler });
		...
		it('creates an autocomplete controller with custom Tracker service', () => {
			const customTracker = new Tracker({ siteId: 'custom' });

			const controller = createAutocompleteController(createConfig, { tracker: customTracker });
