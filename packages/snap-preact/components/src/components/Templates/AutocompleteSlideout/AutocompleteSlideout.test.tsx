...
import { AutocompleteSlideout, AutocompleteSlideoutProps } from '../AutocompleteSlideout/AutocompleteSlideout';
...
describe('AutocompleteSlideout Component', () => {
	...
	it('does not render if input have not been focused', async () => {
		...
		const args: AutocompleteSlideoutProps = {
			theme,
			controller,
			input: controller.config.selector,
		};
		const rendered = render(<AutocompleteSlideout {...args} />);

		const slideout = rendered.container.querySelector('.ss__autocomplete-slideout');
	...
	it('can set titles', async () => {
		...
		const rendered = render(<AutocompleteSlideout {...args} />, { container });
