(self.webpackChunk_searchspring_snap_preact_components = self.webpackChunk_searchspring_snap_preact_components || []).push([
	[839],
	{
		'../../node_modules/@storybook/components/dist/esm/tooltip/WithTooltip.js'(__unused_webpack_module, __webpack_exports__, __webpack_require__) {
			'use strict';
			...
			function getOffsetParent(element) {
				...
				return offsetParent &&
					('html' === getNodeName(offsetParent) || ('body' === getNodeName(offsetParent) && 'static' === getComputedStyle(offsetParent).position))
					? window
					: offsetParent ||
						...
						instance = {
							...
							setOptions: function setOptions(setOptionsAction) {
								...
								var orderedModifiers = (function orderModifiers(modifiers) {
									...
									return enums.GM.reduce(function (acc, phase) {
										return acc.concat(
			...
			function mapToStyles(_ref2) {
				...
				if (adaptive) {
					...
					if (
						...
						placement === enums.Mn || ((placement === enums.kb || placement === enums.pG) && variation === enums._N))
					)
						(sideY = enums.sQ),
							(y -=
								(isFixed && offsetParent === win && win.visualViewport ? win.visualViewport.height : offsetParent[heightProp]) - popperRect.height),
							(y *= gpuAcceleration ? 1 : -1);
					if (placement === enums.kb || ((placement === enums.Mn || placement === enums.sQ) && variation === enums._N))
						(sideX = enums.pG),
			...
			const modifiers_offset = {
				...
				fn: function offset(_ref2) {
					var state = _ref2.state,
						...
						data = enums.DD.reduce(function (acc, placement) {
							return (
								(acc[placement] = (function distanceAndSkiddingToXY(placement, rects, offset) {
									var basePlacement = getBasePlacement(placement),
										invertDistance = [enums.kb, enums.Mn].indexOf(basePlacement) >= 0 ? -1 : 1,
									...
									return (
										...
										[enums.kb, enums.pG].indexOf(basePlacement) >= 0 ? { x: distance, y: skidding } : { x: skidding, y: distance }
									);
			...
			function getClientRectFromMixedType(element, clippingParent, strategy) {
				return clippingParent === enums.R9
					? rectToClientRect(
			...
			function detectOverflow(state, options) {
				...
				var _options = options,
					...
					boundary = void 0 === _options$boundary ? enums.WY : _options$boundary,
					_options$rootBoundary = _options.rootBoundary,
					rootBoundary = void 0 === _options$rootBoundary ? enums.R9 : _options$rootBoundary,
					_options$elementConte = _options.elementContext,
					elementContext = void 0 === _options$elementConte ? enums.xf : _options$elementConte,
					...
					paddingObject = mergePaddingObject('number' != typeof padding ? padding : expandToHashMap(padding, enums.OM)),
					altContext = elementContext === enums.xf ? enums.ir : enums.xf,
					...
					elementClientRect = elementContext === enums.xf ? popperClientRect : referenceClientRect,
					overflowOffsets = {
				...
				if (elementContext === enums.xf && offsetData) {
					var offset = offsetData[placement];
					Object.keys(overflowOffsets).forEach(function (key) {
						var multiply = [enums.pG, enums.sQ].indexOf(key) >= 0 ? 1 : -1,
							axis = [enums.Mn, enums.sQ].indexOf(key) >= 0 ? 'y' : 'x';
					...
					if (popperOffsets) {
						if (checkMainAxis) {
							var _offsetModifierState$,
								mainSide = 'y' === mainAxis ? enums.Mn : enums.kb,
								altSide = 'y' === mainAxis ? enums.sQ : enums.pG,
								...
								minLen = variation === enums.ni ? referenceRect[len] : popperRect[len],
								maxLen = variation === enums.ni ? -popperRect[len] : -referenceRect[len],
								...
								offsetModifierValue =
									null != (_offsetModifierState$ = null == offsetModifierState ? void 0 : offsetModifierState[mainAxis]) ? _offsetModifierState$ : 0,
								tetherMax = offset + maxOffset - offsetModifierValue,
								preventedOffset = within(
									tether ? math_min(min, offset + minOffset - offsetModifierValue - clientOffset) : min,
									offset,
						...
						if (checkAltAxis) {
							var _offsetModifierState$2,
								_mainSide = 'x' === mainAxis ? enums.Mn : enums.kb,
								_altSide = 'x' === mainAxis ? enums.sQ : enums.pG,
								_offset = popperOffsets[altAxis],
								_len = 'y' === altAxis ? 'height' : 'width',
								_min = _offset + overflow[_mainSide],
								_max = _offset - overflow[_altSide],
								isOriginSide = -1 !== [enums.Mn, enums.kb].indexOf(basePlacement),
								_offsetModifierValue =
									null != (_offsetModifierState$2 = null == offsetModifierState ? void 0 : offsetModifierState[altAxis]) ? _offsetModifierState$2 : 0,
			...
			const modifiers_arrow = {
				...
				fn: function arrow(_ref) {
					var _state$modifiersData$,
						...
						len = [enums.kb, enums.pG].indexOf(basePlacement) >= 0 ? 'height' : 'width';
						...
						var paddingObject = (function toPaddingObject(padding, state) {
								return mergePaddingObject(
									'number' !=
										...
										? padding
										: expandToHashMap(padding, enums.OM)
								);
							})(options.padding, state),
							arrowRect = getLayoutRect(arrowElement),
							minProp = 'y' === axis ? enums.Mn : enums.kb,
							maxProp = 'y' === axis ? enums.sQ : enums.pG,
			...
			function isAnySideFullyClipped(overflow) {
				return [enums.Mn, enums.pG, enums.sQ, enums.kb].some(function (side) {
									...
									function () {
										Object.keys(state.elements).forEach(function (name) {
											var element = state.elements[name],
												attributes = state.attributes[name] || {},
												style = Object.keys(state.styles.hasOwnProperty(name) ? state.styles[name] : initialStyles[name]).reduce(function (
													style,
													property
												) {
								...
								if (!state.modifiersData[name]._skip) {
									for (
													...
													getBasePlacement(placement) === enums.qZ
														? (function computeAutoPlacement(state, options) {
																...
																var _options = options,
																	...
																	allowedAutoPlacements = void 0 === _options$allowedAutoP ? enums.DD : _options$allowedAutoP,
																	variation = getVariation(placement),
																	placements = variation
																		? flipVariations
																			? enums.Ol
																			: enums.Ol.filter(function (placement) {
										...
										var placement = placements[i],
											...
											isStartVariation = getVariation(placement) === enums.ni,
											isVertical = [enums.Mn, enums.sQ].indexOf(_basePlacement) >= 0,
											len = isVertical ? 'width' : 'height',
											overflow = detectOverflow(state, { placement, boundary, rootBoundary, altBoundary, padding }),
											mainVariationSide = isVertical ? (isStartVariation ? enums.pG : enums.kb) : isStartVariation ? enums.sQ : enums.Mn;
			...
			var TooltipContext = compat_module.default.createContext({}),
				...
				Tooltip = (function (_Component) {
					function Tooltip() {
						...
						return (
							...
							(_this.getArrowProps = function (props) {
								return (
									void 0 === props && (props = {}),
									(0, esm_extends.A)({}, props, { style: (0, esm_extends.A)({}, props.style, _this.props.arrowProps.style) })
								);
							}),
							(_this.getTooltipProps = function (props) {
								return (
									...
									(0, esm_extends.A)(
										...
										{ style: (0, esm_extends.A)({}, props.style, _this.props.style) }
									)
								);
							}),
							(_this.contextValue = {
					...
					(0, inheritsLoose.A)(Tooltip, _Component);
			...
			var TooltipTrigger = (function (_Component) {
				function TooltipTrigger() {
					...
					return (
						...
						(_this.showTooltip = function (_ref) {
							...
							_this.props.followCursor && (state = (0, esm_extends.A)({}, state, { pageX, pageY })),
				...
				(0, inheritsLoose.A)(TooltipTrigger, _Component);
				...
				return (
					...
					(_proto.render = function render() {
						var _this2 = this,
							...
							restProps = (0, objectWithoutPropertiesLoose.A)(_this$props, [
								'children',
								'tooltip',
								'placement',
								'trigger',
					...
					function (_ref2) {
						...
						return {
							...
							borderTopColor: match(
								'top',
								placement,
								theme.color[color] || color || 'light' === theme.base ? (0, utils.Zt)(theme.background.app) : (0, utils.nV)(theme.background.app),
								'transparent'
							),
