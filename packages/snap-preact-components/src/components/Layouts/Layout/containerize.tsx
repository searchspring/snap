...
import { Layout, LayoutFuncData } from '.';
import { LayoutElement, LayoutTypes } from '.';
...
export function containerize(
	data: LayoutFuncData<AutocompleteController | RecommendationController | SearchController>,
						...
						if (mappedProps) {
							Object.keys(mappedProps).map((propName) => {
								...
								if ((componentMapping.layoutProps || []).includes(propName)) {
									const layoutProp = propValue as LayoutTypes;
									if (typeof propValue == 'function' || Array.isArray(propValue) || typeof propValue == 'object') {
										...
										mappedProps[propName as keyof typeof mappedProps] = <Layout {...data} layout={layoutProp} />;
									}
								}
							});
						}
